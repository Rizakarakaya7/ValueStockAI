...
# Extractors (Tamamen YFinance altyapısına geçildi)
from data_pipeline.extractors.yf_financials import (
    YFinanceFinancialsExtractor
)
...
class ValuationOrchestrator:
    ...
    def __init__(
        ...
        self.extractor_yf_fins = YFinanceFinancialsExtractor()
    ...
    async def _step_extract_data(
        ...
        # Yahoo Finance üzerinden bilançoları çeken metod bağlandı
        yf_financials_task = (
            self.extractor_yf_fins.fetch_financials(
                ticker=ctx.ticker
            )
        )
    ...
    def _step_normalize_data(
        ...
        # YFinance verisine uygun normalizer fonksiyonu çağrılıyor
        df, meta = (
            self.normalizer
            .normalize_yfinance_payload(
                ticker=ctx.ticker,
                raw_data=raw_data,
                extraction_timestamp_utc=(
                    ctx.metadata[
