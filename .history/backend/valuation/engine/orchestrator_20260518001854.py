...
import pandas as pd
import numpy as np  # <-- EKLENDİ: Numpy tiplerini temizlemek için
...
class ValuationOrchestrator:
    ...
    async def execute_job(self, job_id: str, ticker: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"[{job_id}] PIPELINE TAMAMLANDI: {ticker} | Intrinsic Value: {final_payload['intrinsic_value']}")
    ...
    # =====================================================
    # JSON SANITIZER (YENİ EKLENDİ)
    # =====================================================
    def _sanitize_for_json(self, obj: Any) -> Any:
        """
        Pandas/Numpy objelerini JSON (ve FastAPI Pydantic) ile uyumlu standart 
        Python tiplerine (str, int, float, None) dönüştüren recursive temizleyici.
        """
        if isinstance(obj, dict):
            # Eğer Dictionary anahtarı (key) bir Pandas Period ise, string'e çevir.
            return {
            ...
            return str(obj)
        elif pd.isna(obj): # pd.NA, np.nan vb. durumlarında null(None) dönsün
            return None
    ...
    def _build_final_payload(self, ctx: PipelineContext) -> Dict[str, Any]:
        ...
        # BURADA SANITIZER ÇALIŞTIRILIYOR
        ...
        return {
            ...
            "base_intrinsic_value": sanitized_metadata.get("base_intrinsic_value"),
            "intrinsic_value": final_value,
            "upside_potential_pct": upside_pct,
            "valuation_metadata": sanitized_metadata, # Artık tamamen JSON uyumlu!
