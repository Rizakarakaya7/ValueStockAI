...
# Extractors
from data_pipeline.extractors.is_yatirim import (
    IsYatirimExtractor
)
...
class ValuationOrchestrator:
    ...
    async def _step_extract_data(
        ...
        is_yatirim_task = (
            self.extractor_is.fetch_financials(
                ctx.ticker,
                requested_periods,
                ctx.config.reporting_template.value
            )
        )
    ...
    def _step_normalize_data(
        ...
        df, meta = (
            self.normalizer
            .normalize_isyatirim_payload(
                ticker=ctx.ticker,
                raw_data=ctx.metadata[
                    "raw_financials"
                ],
                actual_periods=ctx.metadata[
                    "actual_periods"
                ],
                extraction_timestamp_utc=(
                    ctx.metadata[
