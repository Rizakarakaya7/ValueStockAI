...
class CapexDeductor:
    ...
    def apply_capex_deduction(
        ...
        # HATA DÜZELTİLDİ: Pandas'ın "Q-DEC" gibi dönütlerini yakalamak için startswith kullanıldı.
        if not hasattr(df.columns, 'freqstr') or not df.columns.freqstr.startswith(self.EXPECTED_FREQ):
            raise PipelineContractError(f"[{ticker}] DataFrame columns PeriodIndex(freq='Q') formatında değil. Mevcut freq: {getattr(df.columns, 'freqstr', 'Yok')}")
        ...
        if not existing_keys:
            ...
            # Type-Level Stock/Flow Validation
        ...
        # 5. OWNER'S EARNINGS HESAPLAMASI
        ...
        metadata["methodology"]["owners_earnings_method"] = "proxy_da_based"
